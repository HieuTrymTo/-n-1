...
<body>
    ...
    <div class="result">
        <?php
                ...
                $sql = "SELECT * FROM product WHERE productsName LIKE '%$search%' OR productsDesc LIKE '%$search%' ORDER BY productsOrder;";
                $stmt = mysqli_stmt_init($conn);
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                    echo $search;
                    ...
                    if ($count > 1) {
                        ...
                        <div class='notification'>
                            <p>".$count." Search Results For '".$search."'</p>
                        </div>";
                    } else {
                        echo "
                        <div class='notification'>
                            <p>".$count." Search Result For '".$search."'</p>
                        </div>";
                    }
    ...
    <div class="product-card">
        <?php
                ...
                $sql = "SELECT * FROM product WHERE productsName LIKE '%$search%' OR productsDesc LIKE '%$search%' LIMIT $start,$num_per_page;";
                $stmt = mysqli_stmt_init($conn);
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                    echo "SQL statement failed";
                } else {
    ...
    <?php
        $pagePrev = $page - 1;
        $pageNext = $page + 1;
        $sql = "SELECT * FROM product WHERE productsName LIKE '%$search%' OR productsDesc LIKE '%$search%'";
        $stmt = mysqli_stmt_init($conn);
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            ...
            <div id="custom-select"> 
                ...
                if ($page == floor($totalpage) && floor($totalpage)> 1) {
                    ?>
                        <a href="product-search.php?page=<?php echo $pagePrev?>&search=<?php echo $search?>">Prev</a>
                        <div class="optionvalue">
                            <select onchange="location = this.value;" class="">
                                <?php
                                for ($i = 1; $i <= $totalpage; $i++) {
                                    echo "<option value=product-search.php?page=".$i."&search=".$search.">$i</option>";
                                }
                                ?>
                            </select>
                    ...
                    ?>
                        ...
                        <a href="product-search.php?page=<?php echo $pageNext?>&search=<?php echo $search?>">Next</a>
                    <?php
    ...
    <script>  
        $("div.optionvalue select").val("product-search.php?page=<?php echo $page;?>&search=<?php echo $search;?>");
    </script>
