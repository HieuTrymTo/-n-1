...
<body>
    ...
    <div class="result">
        <?php
                    ...
                    $sql = "SELECT * FROM product WHERE categoriesId = ?;";
                    $stmt = mysqli_stmt_init($conn);
                    if (!mysqli_stmt_prepare($stmt, $sql)) {
                        ...
                        if (!mysqli_stmt_prepare($stmt, $sql)) {
                            ...
                            if ($count > 1) {
                                ...
                                <div class='notification'>
                                    <p>".$count." Products On '".$name."'</p>
                                </div>";
                            } else {
                                echo "
                                <div class='notification'>
                                    <p>".$count." Product On '".$name."'</p>
                                </div>";
                            }
                        }
                    }
    ...
    <div class="product-card">
        <?php
            ...
            $sql = "SELECT * FROM product WHERE categoriesId=? ORDER BY productsOrder LIMIT $start,$num_per_page ";
            $stmt = mysqli_stmt_init($conn);
            if (!mysqli_stmt_prepare($stmt, $sql)) {
                echo "SQL statement failed1";
            } else {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    // if ($row['productsCategories'] === $name) {
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            ...
            <div id="custom-select"> 
                ...
                if ($page == floor($totalpage) && floor($totalpage)> 1) {
                    ?>
                        <a href="product-category.php?page=<?php echo $pagePrev?>&category=<?php echo $pid?>">Prev</a>
                        <div class="optionvalue">
                            <select onchange="location = this.value;" class="">
                                <?php
                                for ($i = 1; $i <= $totalpage; $i++) {
                                    echo "<option value=product-category.php?page=".$i."&category=".$pid.">$i</option>";
                                }
                                ?>
                            </select>
                    ...
                    ?>
                        ...
                        <a href="product-category.php?page=<?php echo $pageNext?>&category=<?php echo $pid?>">Next</a>
                    <?php
                    ...
                    ?>
                        ...
                        <div class="optionvalue">
                            <select onchange="location = this.value;" class="">
                                <?php
                                for ($i = 1; $i <= $totalpage; $i++) {
                                    echo "<option value=product-category.php?page=".$i."&category=".$pid."s>$i</option>";
                                }
                                ?>
                            </select>
                            <p>&ensp;of <?php echo floor($totalpage); ?></p>
    ...
    <script>  
        $("div.optionvalue select").val("product-category.php?page=<?php echo $page?>&category=<?php echo $pid?>");
    </script>
